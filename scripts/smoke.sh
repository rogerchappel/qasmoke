...
node dist/cli.js run fixtures/format --provider fixture --baseline .tmp/report.json --max-score-drop 0 --output .tmp/format-report.json > .tmp/format-run.json
...
node -e "const fs=require('fs'); const report=JSON.parse(fs.readFileSync('.tmp/format-report.json','utf8')); if(!report.pass || !report.regression?.pass) process.exit(1);"
